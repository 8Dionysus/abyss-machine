...
resource launch route: abyss-machine resource launch --class CLASS --kind KIND -- COMMAND; the single canonical host launch path applies relative CPUWeight/IOWeight, cooperative startup reserve, and bounded runtime peak learning through user systemd-run to new processes only; AllowedCPUs and thread-count environment are applied only when the live CPU owner marks placement required or reports CPUs that must be avoided, while an advisory cpuset leaves all scheduler-visible CPUs available; use --no-thermal-sample for quick dry-run checks that should rely on the latest process thermal plan; use --success-on-block for unattended timers that should report a clean skip when a real owner or safety contract blocks
resource limits rule: static MemoryHigh, MemoryMax, MemorySwapMax, CPUQuota, advisory CPU affinity, and advisory thread-count caps are not set; pressure and swap occupancy do not assign workload importance; existing user/game/work/stack/persistent-model/warm-AI processes are never killed, throttled, re-affinitized, migrated, stopped, or demoted by this route
