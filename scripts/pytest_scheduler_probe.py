"""Pytest plugin for exact non-authoritative scheduler experiment receipts."""
...
REPORT_LOG_ENV = "ABYSS_MACHINE_PYTEST_REPORT_LOG"
...
def pytest_xdist_node_collection_finished(node: Any, ids: list[str]) -> None:
    gateway = getattr(node, "gateway", node)
    _write(
        {
            "event": "collection",
            "worker": str(getattr(gateway, "id", "unknown")),
            "nodeids": list(ids),
        }
    )
