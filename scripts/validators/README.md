...
plus `doctor` power/cooling, storage/process, and snapshot/observability status
probe collection live in `abyss_machine.doctor_adapters`; report writes,
machine-report artifact reads, machine-report input collection, safe repair
orchestration, and machine-report latest/history/markdown writes are also
routed through `doctor_adapters`;
