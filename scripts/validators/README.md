...
`web-ext` execution, loopback HTTP probe serving, subprocess cleanup, probe
polling, and public-safe WebExtension selftest result assembly live in
...
Policy file reads, framed native-host byte IO, remaining browser AT-SPI,
focused-browser, browser-context, and browser-privacy live probes, `pyatspi`
access, listener registration, text reads, systemd status reads,
