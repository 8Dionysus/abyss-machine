...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run abyss-machine validation lanes.")
    parser.add_argument("--mode", default="source-fast", choices=validation_lanes.lane_ids())
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        steps = validation_lanes.lane_command_sequence(args.mode)
    except validation_lanes.ManifestError as exc:
        print(f"[fail] validation lane failed to load: {exc}", flush=True)
        return 1
    for step in steps:
