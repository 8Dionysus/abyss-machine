...
7. [docs/publication/PUBLICATION_BOUNDARY.md](docs/publication/PUBLICATION_BOUNDARY.md)
8. [kag/README.md](kag/README.md)
...
`abyss_machine.typing_codex_semantics`; zsh submitted-command hook marker/source
detection, function-probe command/result projection, status document assembly,
selftest probe/command/run-result/document assembly, and recent-record
projection live in `abyss_machine.typing_shell_adapters`; VS Code
editor-extension selftest plan/probe/event projection, selftest document
assembly, latest-status readmodel assembly, callback selftest probe/document
assembly, disposable runtime prep, subprocess lifecycle cleanup, callback
polling through supplied ports, and public-safe result documents live in
...
execution, zsh hook and `.zshrc` reads/stats, shell subprocess execution, editor
path/env/write/probe/store/latest-file binding, state/latest writes,
latest/history writes, browser selftest callback binding, and command rendering;
...
remain with workload owners instead of a resident host controller.
Request-specific storage, thermal-map/CPU-route, mode, and game-guard proof is
collected in a bounded-age, two-wave pre-admission DAG outside the startup lock:
the direct thermal map and mode plan precede their exact route projection. The
thermal receipt carries the exact requested route into the final plan and
fails closed when direct sensor evidence is unavailable or the route identity
does not match. Process attribution and desktop/compositor inspection remain
...
activation-gap handoff route contracts are module-owned while broader
