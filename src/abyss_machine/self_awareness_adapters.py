...
@dataclass(frozen=True)
class SelfAwarenessCapabilitiesPaths:
    ...
    ai_tts_success_latest: Path
    ai_llm_registry_latest: Path
    llm_resident_status_latest: Path
    llm_resident_monitor_latest: Path
    llm_resident_digest_latest: Path
    llm_resident_micro_latest: Path
    llm_resident_evals_latest: Path
    llm_resident_candidates_latest: Path
    llm_workhorse_preflight_latest: Path
    llm_workhorse_pack_latest: Path
    llm_workhorse_review_latest: Path
    llm_workhorse_validate_latest: Path
...
@dataclass(frozen=True)
class SelfAwarenessCapabilitiesUrls:
    prometheus: str
    grafana: str
    loki: str
    alertmanager: str
...
@dataclass(frozen=True)
class SelfAwarenessCapabilitiesInputPort:
    stack_observability: RefreshDocumentPort
    container_health: RefreshDocumentPort
    http_json: Callable[..., dict[str, Any]]
    langchain_api_probe: NoArgDocumentPort
    stack_closure_external_evidence: NoArgDocumentPort
    trace_backend_probe: DocumentBuilderPort
    grafana_datasource_probe: DocumentBuilderPort
    stack_memory_space_probe: NoArgDocumentPort
    module_available: ModuleAvailablePort
    ai_capabilities: NoArgDocumentPort
    ai_llm_registry: RefreshDocumentPort
    load_latest_json: LatestJsonReaderPort
    rag_validate: RefreshDocumentPort
    nervous_brief: RefreshDocumentPort


@dataclass(frozen=True)
class SelfAwarenessCapabilitiesContractPort:
    resident_worker_detail: DocumentBuilderPort
    resident_worker_detail_complete: DocumentCompletePort
    ai_multimodal_detail: DocumentBuilderPort
    ai_multimodal_detail_complete: DocumentCompletePort
    governance_gate_detail: DocumentBuilderPort
    governance_gate_detail_complete: DocumentCompletePort
    llm_escalation_detail: DocumentBuilderPort
    llm_escalation_detail_complete: DocumentCompletePort
    capability_item: DocumentBuilderPort
    requirement_item: DocumentBuilderPort
    requirements_document: DocumentBuilderPort
    requirements_with_probe_readiness: DocumentBuilderPort


@dataclass(frozen=True)
class SelfAwarenessCapabilitiesPersistencePort:
    write_latest_and_history: WriteLatestHistoryPort


@dataclass(frozen=True)
class SelfAwarenessStatusPort:
    load_latest_json: LatestJsonReaderPort
...
@dataclass(frozen=True)
class SelfAwarenessExportPaths:
    artifacts: Mapping[str, Path]
    export_latest: Path
    export_history_root: Path


@dataclass(frozen=True)
class SelfAwarenessExportRuntimePort:
    load_latest_json: LatestJsonReaderPort
    daily_jsonl_path: DailyJsonlPathPort
...
@dataclass(frozen=True)
class SelfAwarenessExportRefreshPort:
    requirements: RefreshDocumentPort
    ...
    coverage_audit: RefreshDocumentPort
    activation_smoke: RefreshDocumentPort
    autolink: RefreshDocumentPort
    completion_audit: RefreshDocumentPort
    ...
    refresh_working_stack_dependents: RefreshDocumentPort


@dataclass(frozen=True)
class SelfAwarenessExportContractPort:
    trace_context_fallback_complete: DocumentCompletePort
    requirement_probes_cover_requirements: DocumentBuilderPort
    autolink_complete: DocumentCompletePort
    activation_smoke_needs_refresh: DocumentBuilderPort
    entity_event_document_map_issues: DocumentBuilderPort
    entity_event_document_map_complete: DocumentBuilderPort
    completion_route_packet_issues: DocumentBuilderPort
    export_stack_handoff: DocumentBuilderPort
    requirement_probes_export_ready: DocumentCompletePort
    ...
    memory_space_freshness_handoff: DocumentBuilderPort
    response_entity_event_document_context_complete: DocumentCompletePort
    working_stack_link_integrity_complete: DocumentCompletePort
    link_integrity_matches_working_stack: DocumentBuilderPort
    working_stack_activation_synthetic_proof_complete: DocumentCompletePort


@dataclass(frozen=True)
class SelfAwarenessExportPersistencePort:
    write_latest_and_history: WriteLatestHistoryPort
...
def export_artifact_entry(
    name: str,
    path: Path,
    schema: str,
    *,
    runtime_port: SelfAwarenessExportRuntimePort,
) -> dict[str, Any]:
    data = runtime_port.load_latest_json(path, schema)
    ...
    daily_path = runtime_port.daily_jsonl_path(path.parent)
    return {
        "name": name,
        "path": str(path),
        "history_path": str(daily_path),
        "exists": exists,
        "history_exists": runtime_port.path_exists(daily_path),
        ...
        "size_bytes": runtime_port.path_stat(path).st_size if exists else None,
        "sha256": runtime_port.path_sha256(path) if exists else None,
        "evidence_ref": {"path": str(path), "schema": data.get("schema") or schema},
    }


def export_artifact_status_lists(exported: Mapping[str, Any]) -> tuple[list[str], list[str]]:
    missing = sorted(
        str(name)
...
def refresh_exported_artifacts(
    exported: dict[str, Any],
    artifact_specs: Mapping[str, tuple[Path, str]],
    names: Iterable[str],
    *,
    runtime_port: SelfAwarenessExportRuntimePort,
    ...
    for name in names:
        ...
        if not spec:
            continue
        path, schema = spec
        exported[name] = export_artifact_entry(name, path, schema, runtime_port=runtime_port)
    artifact_list = sorted(
        [artifact for artifact in exported.values() if isinstance(artifact, dict)],
        key=lambda item: str(item.get("name") or ""),
    )
    missing, malformed = export_artifact_status_lists(exported)
...
def run_status(
    *,
    ...
    version: str,
    generated_at: str,
    latest_paths: Mapping[str, Path],
    port: SelfAwarenessStatusPort,
) -> dict[str, Any]:
    SCHEMA_PREFIX = schema_prefix
    VERSION = version
    load_latest_json = port.load_latest_json
    self_awareness_body_closure_status = port.body_closure_status
    self_awareness_working_stack_activation_gap_route = port.activation_gap_route
    self_awareness_working_stack_activation_gap_route_complete = port.activation_gap_route_complete
    self_awareness_paths = port.paths_document
    nested_get = _nested_get

    latest_paths = dict(latest_paths)
    readmodel_specs = readmodel_latest_specs(
        schema_prefix=SCHEMA_PREFIX,
        paths=latest_paths,
    )
    status_specs = status_latest_specs(
        schema_prefix=SCHEMA_PREFIX,
        paths=latest_paths,
    )
    latests = load_latest_documents(
        status_specs,
        load_latest_json=load_latest_json,
    )
    completion_audit_latest = latests["completion_audit"]
    body_closure = self_awareness_body_closure_status()
    missing = missing_latest_document_names({
        spec.name: latests.get(spec.name, {})
        for spec in readmodel_specs
    })
    ...
    def summary_value(doc: dict[str, Any], key: str) -> Any:
        return nested_get(doc, ["summary", key])

    open_stack_requirements = first_present(
        nested_get(latests["autolink"], ["summary", "open_stack_requirements"]),
        nested_get(latests["stack_closure_dossier"], ["summary", "open_stack_requirements"]),
        nested_get(latests["requirement_probes"], ["summary", "open"]),
    )
    autolink_organ_rows = latests["autolink"].get("organ_links") if isinstance(latests["autolink"].get("organ_links"), list) else []
    autolink_requirement_rows = latests["autolink"].get("stack_requirement_links") if isinstance(latests["autolink"].get("stack_requirement_links"), list) else []
    activation_entries = nested_get(latests["stack_closure_dossier"], ["working_stack_activation_dossier", "entries"])
    ...
    activation_smoke_rows = latests["activation_smoke"].get("rows") if isinstance(latests["activation_smoke"].get("rows"), list) else []
    activation_smoke_by_service = {
        str(row.get("service")): row
        for row in activation_smoke_rows
        if isinstance(row, dict) and row.get("service")
    }
    requirement_rows = latests["requirements"].get("requirements") if isinstance(latests["requirements"].get("requirements"), list) else []
    requirement_by_id = {
        str(row.get("id")): row
        for row in requirement_rows
        if isinstance(row, dict) and row.get("id")
    }
    stack_closure_entries = latests["stack_closure_dossier"].get("entries") if isinstance(latests["stack_closure_dossier"].get("entries"), list) else []
    stack_closure_by_id = {
        str(row.get("requirement_id")): row
        for row in stack_closure_entries
        if isinstance(row, dict) and row.get("requirement_id")
    }
    autolink_service_ids = nested_get(latests["autolink"], ["summary", "service_ids"])
    if not isinstance(autolink_service_ids, list):
        autolink_service_ids = sorted({
            str(row.get("service"))
            for row in autolink_organ_rows
            if isinstance(row, dict) and row.get("service")
        })
    open_potential_services = sorted({
        str(row.get("service"))
        for row in autolink_organ_rows
        if isinstance(row, dict) and row.get("service") and row.get("usage_gap")
    })
    autolink_requirement_ids = nested_get(latests["autolink"], ["summary", "requirement_ids"])
    if not isinstance(autolink_requirement_ids, list):
        autolink_requirement_ids = sorted({
            str(row.get("requirement_id"))
            for row in autolink_requirement_rows
            if isinstance(row, dict) and row.get("requirement_id")
        })
    open_potential_rows = status_open_potential_rows(
    ...
    open_stack_requirement_rows = status_open_stack_requirement_rows(
    ...
    stack_usage_status = "complete" if completion_stack_usage_closure_complete is True else ("incomplete" if completion_green_but_incomplete is True else "unknown")
    body_status = str(body_closure.get("status") or "unknown")
    status_value = readmodel_status
    if readmodel_status == "ready" and completion_stack_usage_closure_complete is True and body_closure.get("complete") is True:
        status_value = "complete"
    elif readmodel_status == "ready" and body_closure.get("complete") is not True:
        status_value = "watch"
    elif readmodel_status == "ready" and completion_green_but_incomplete is True:
        status_value = "ready_with_open_stack_usage"
    summary = {
        "status": status_value,
        "readmodel_status": readmodel_status,
        "stack_usage_status": stack_usage_status,
        "body_status": body_status,
        "body_closure_complete": body_closure.get("complete"),
        "body_open_routes": nested_get(body_closure, ["summary", "response_routes"]),
        "body_watch_sources": nested_get(body_closure, ["summary", "watch_sources"]),
        "missing_or_invalid": missing,
        "events": nested_get(latests["events"], ["summary", "events"]),
        "episodes": nested_get(latests["episodes"], ["summary", "episodes"]),
        "reaction_candidates": nested_get(latests["alerts"], ["summary", "reaction_candidates"]),
        "capabilities": nested_get(latests["capabilities"], ["summary", "capabilities"]),
        "requirements": nested_get(latests["requirements"], ["summary", "requirements"]),
        "requirement_probes": nested_get(latests["requirement_probes"], ["summary", "probes"]),
        "requirement_probes_open": nested_get(latests["requirement_probes"], ["summary", "open"]),
        "open_stack_requirements": open_stack_requirements,
        "stack_closure_dossier_open": nested_get(latests["stack_closure_dossier"], ["summary", "open_stack_requirements"]),
        "trace_context_fallback_ok": latests["trace_context"].get("ok"),
        "trace_context_fallback_status": latests["trace_context"].get("status"),
        "trace_context_stack_requirement_not_closed_by_fallback": nested_get(latests["trace_context"], ["summary", "stack_requirement_not_closed_by_fallback"]),
        "trace_context_bounded_links": nested_get(latests["trace_context"], ["summary", "bounded_trace_context_links"]),
        "failure_modes": nested_get(latests["failure_matrix"], ["summary", "failure_modes"]),
        "working_stack_organs": nested_get(latests["working_stack"], ["summary", "organs"]),
        "working_stack_usage_gaps": nested_get(latests["working_stack"], ["summary", "usage_gaps"]),
        "coverage_audit_ok": latests["coverage_audit"].get("ok"),
        "coverage_blocked_stack_owned": nested_get(latests["coverage_audit"], ["summary", "blocked_stack_owned"]),
        "coverage_incomplete": nested_get(latests["coverage_audit"], ["summary", "incomplete"]),
        "activation_smoke_ok": latests["activation_smoke"].get("ok"),
        "activation_smoke_rows": nested_get(latests["activation_smoke"], ["summary", "rows"]),
        "activation_smoke_rows_ok": nested_get(latests["activation_smoke"], ["summary", "rows_ok"]),
        "activation_smoke_open_activation_gaps": nested_get(latests["activation_smoke"], ["summary", "open_activation_gaps"]),
        "open_potential_activation_gap_routes": sum(1 for item in open_potential_rows if self_awareness_working_stack_activation_gap_route_complete(item.get("activation_gap_route"))),
        "open_potential_activation_gap_classifications": sorted({
            str(item.get("activation_gap_classification"))
            for item in open_potential_rows
            if item.get("activation_gap_classification")
        }),
        "autolink_ok": latests["autolink"].get("ok"),
        "autolink_organ_links": nested_get(latests["autolink"], ["summary", "organ_links"]),
        "autolink_organ_links_complete": nested_get(latests["autolink"], ["summary", "organ_links_complete"]),
        "autolink_stack_requirement_links": nested_get(latests["autolink"], ["summary", "stack_requirement_links"]),
        "autolink_stack_requirement_links_complete": nested_get(latests["autolink"], ["summary", "stack_requirement_links_complete"]),
        "autolink_synthetic_scenarios": nested_get(latests["autolink"], ["summary", "synthetic_scenarios"]),
        "autolink_synthetic_scenarios_complete": nested_get(latests["autolink"], ["summary", "synthetic_scenarios_complete"]),
        "autolink_state_changed": nested_get(latests["autolink"], ["summary", "state_changed"]),
        "autolink_state_digest": latests["autolink"].get("state_digest"),
        "autolink_service_ids": autolink_service_ids,
        "open_potential_services": open_potential_services,
        "open_stack_requirement_ids": autolink_requirement_ids,
        "investigation_ok": latests["investigate"].get("ok"),
        "replay_ok": latests["replay"].get("ok"),
        "export_ok": latests["export"].get("ok"),
        "probe_ok": latests["probe"].get("ok"),
        "cycle_ok": latests["cycle"].get("ok"),
        "cycle_steps": nested_get(latests["cycle"], ["summary", "steps"]),
        "validate_ok": latests["validate"].get("ok"),
        "completion_audit_ok": completion_audit_latest.get("ok"),
        "stack_usage_closure_complete": completion_stack_usage_closure_complete,
        "validator_green_but_stack_usage_incomplete": completion_green_but_incomplete,
        "completion_actions": nested_get(completion_audit_latest, ["summary", "completion_actions"]),
        "top_completion_action_id": nested_get(completion_audit_latest, ["summary", "top_completion_action_id"]),
        "top_completion_priority_class": nested_get(completion_audit_latest, ["summary", "top_completion_priority_class"]),
        "completion_drilldowns": nested_get(completion_audit_latest, ["action_backlog", "summary", "drilldowns"]),
        "completion_drilldowns_complete": nested_get(completion_audit_latest, ["action_backlog", "summary", "drilldowns_complete"]),
        "top_completion_drilldown_id": nested_get(completion_audit_latest, ["action_backlog", "summary", "top_action_drilldown_id"]),
        "top_completion_drilldown_complete": nested_get(completion_audit_latest, ["action_backlog", "summary", "top_action_drilldown_complete"]),
        "completion_routes": nested_get(completion_audit_latest, ["completion_route_map", "summary", "routes"]),
        "next_completion_route_id": nested_get(completion_audit_latest, ["completion_route_map", "summary", "next_route_id"]),
        "next_completion_route_path": nested_get(completion_audit_latest, ["completion_route_map", "summary", "next_route_path"]),
        "next_completion_route_action_ids": nested_get(completion_audit_latest, ["completion_route_map", "summary", "next_route_action_ids"]),
        "completion_route_packets": nested_get(completion_audit_latest, ["completion_route_packets", "summary", "packets"]),
        "completion_route_packet_actions": nested_get(completion_audit_latest, ["completion_route_packets", "summary", "covered_actions"]),
        "completion_route_packet_automation_ready": nested_get(completion_audit_latest, ["completion_route_packets", "summary", "automation_ready"]),
        "top_completion_route_packet_id": nested_get(completion_audit_latest, ["completion_route_packets", "summary", "top_packet_id"]),
        "entity_event_document_entities": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "entities"]),
        "entity_event_document_events": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "events"]),
        "entity_event_document_documents": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "documents"]),
        "entity_event_document_stack_organs": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "stack_organs"]),
        "entity_event_document_machine_bridges": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "machine_bridges"]),
        "entity_event_document_body_surfaces": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "body_surfaces"]),
        "entity_event_document_automation_ready": nested_get(completion_audit_latest, ["entity_event_document_map", "summary", "automation_ready"]),
    }
    return {
        "schema": f"{SCHEMA_PREFIX}_self_awareness_status_v1",
        "version": VERSION,
        "generated_at": generated_at,
        "ok": not missing,
        ...
        "summary": summary,
        "open_potential": {
            "schema": f"{SCHEMA_PREFIX}_self_awareness_open_potential_status_v1",
            "services": len(open_potential_rows),
            "activation_gap_routes": sum(1 for item in open_potential_rows if self_awareness_working_stack_activation_gap_route_complete(item.get("activation_gap_route"))),
        ...
        "open_stack_requirements": {
            "schema": f"{SCHEMA_PREFIX}_self_awareness_open_stack_requirements_status_v1",
            "requirements": len(open_stack_requirement_rows),
            "rows": open_stack_requirement_rows,
            "policy": {
                "owner_route": "abyss-stack",
                "handoff_only": True,
                "host_layer_mutates_stack": False,
        ...
        "latest": latest_summary_map(status_specs, latests),
        "tests": {
            "validator": "abyss-machine self-awareness validate --json",
            "e2e": "abyss-machine self-awareness probe --json",
...
def run_capabilities(
    *,
    ...
    version: str,
    generated_at: str,
    paths: SelfAwarenessCapabilitiesPaths,
    urls: SelfAwarenessCapabilitiesUrls,
    write_latest: bool,
    input_port: SelfAwarenessCapabilitiesInputPort,
    contract_port: SelfAwarenessCapabilitiesContractPort,
    persistence_port: SelfAwarenessCapabilitiesPersistencePort,
) -> dict[str, Any]:
    SCHEMA_PREFIX = schema_prefix
    VERSION = version
    STACK_OBSERVABILITY_PROMETHEUS_URL = urls.prometheus
    STACK_OBSERVABILITY_GRAFANA_URL = urls.grafana
    STACK_OBSERVABILITY_LOKI_URL = urls.loki
    SELF_AWARENESS_ALERTMANAGER_URL = urls.alertmanager
    SELF_AWARENESS_TEMPO_URL = urls.tempo
    SELF_AWARENESS_LANGGRAPH_URL = urls.langgraph
    ...
    AI_TTS_EVAL_LATEST_SUCCESS_PATH = paths.ai_tts_success_latest
    AI_LLM_REGISTRY_LATEST_PATH = paths.ai_llm_registry_latest
    AI_LLM_RESIDENT_EVALS_LATEST_PATH = paths.llm_resident_evals_latest
    AI_LLM_RESIDENT_CANDIDATES_LATEST_PATH = paths.llm_resident_candidates_latest
    AI_LLM_WORKHORSE_PREFLIGHT_LATEST_PATH = paths.llm_workhorse_preflight_latest
    AI_LLM_WORKHORSE_PACK_LATEST_PATH = paths.llm_workhorse_pack_latest
    AI_LLM_WORKHORSE_REVIEW_LATEST_PATH = paths.llm_workhorse_review_latest
    AI_LLM_WORKHORSE_VALIDATE_LATEST_PATH = paths.llm_workhorse_validate_latest
    ...
    SELF_AWARENESS_CAPABILITIES_ROOT = paths.capabilities_root
    ...
    self_awareness_grafana_datasource_probe = input_port.grafana_datasource_probe
    self_awareness_stack_memory_space_probe = input_port.stack_memory_space_probe
    ai_capabilities_latest = input_port.ai_capabilities
    ai_llm_registry = input_port.ai_llm_registry
    load_latest_json = input_port.load_latest_json
    rag_validate = input_port.rag_validate
    nervous_brief = input_port.nervous_brief
    nested_get = _nested_get

    self_awareness_resident_worker_detail = contract_port.resident_worker_detail
    self_awareness_resident_worker_detail_complete = contract_port.resident_worker_detail_complete
    self_awareness_ai_multimodal_detail = contract_port.ai_multimodal_detail
    self_awareness_ai_multimodal_detail_complete = contract_port.ai_multimodal_detail_complete
    self_awareness_governance_gate_detail = contract_port.governance_gate_detail
    self_awareness_governance_gate_detail_complete = contract_port.governance_gate_detail_complete
    self_awareness_llm_escalation_detail = contract_port.llm_escalation_detail
    self_awareness_llm_escalation_detail_complete = contract_port.llm_escalation_detail_complete
    self_awareness_capability_item = contract_port.capability_item
    self_awareness_requirement_item = contract_port.requirement_item
    self_awareness_requirements_document = contract_port.requirements_document
    self_awareness_requirements_with_probe_readiness = contract_port.requirements_with_probe_readiness
    write_latest_and_history = persistence_port.write_latest_and_history
    ...
    langgraph_python = input_port.module_available("langgraph")
    ...
    llm_resident_status = load_latest_json(paths.llm_resident_status_latest, f"{SCHEMA_PREFIX}_gemma4_spark_resident_status_v1")
    llm_resident_monitor = load_latest_json(paths.llm_resident_monitor_latest, f"{SCHEMA_PREFIX}_gemma4_spark_resident_monitor_v1")
    llm_resident_digest = load_latest_json(paths.llm_resident_digest_latest, f"{SCHEMA_PREFIX}_gemma4_spark_resident_digest_v1")
    llm_resident_micro = load_latest_json(paths.llm_resident_micro_latest, f"{SCHEMA_PREFIX}_gemma4_spark_resident_micro_tick_v1")
    ...
    capabilities = [
        ...
        self_awareness_capability_item(
            ...
            required=True,
            evidence_refs=[
                evidence(paths.llm_resident_status_latest, llm_resident_status.get("schema")),
                evidence(paths.llm_resident_monitor_latest, llm_resident_monitor.get("schema")),
                evidence(paths.llm_resident_digest_latest, llm_resident_digest.get("schema")),
                evidence(paths.llm_resident_micro_latest, llm_resident_micro.get("schema")),
...
def run_export(
    *,
    ...
    write_latest: bool,
    include_cycle: bool,
    paths: SelfAwarenessExportPaths,
    runtime_port: SelfAwarenessExportRuntimePort,
    refresh_port: SelfAwarenessExportRefreshPort,
    contract_port: SelfAwarenessExportContractPort,
    persistence_port: SelfAwarenessExportPersistencePort,
) -> dict[str, Any]:
    SCHEMA_PREFIX = schema_prefix
    VERSION = version
    SELF_AWARENESS_CAPABILITIES_LATEST_PATH = paths.artifacts["capabilities"]
    SELF_AWARENESS_REQUIREMENTS_LATEST_PATH = paths.artifacts["requirements"]
    SELF_AWARENESS_REQUIREMENT_PROBES_LATEST_PATH = paths.artifacts["requirement_probes"]
    SELF_AWARENESS_STACK_CLOSURE_DOSSIER_LATEST_PATH = paths.artifacts["stack_closure_dossier"]
    SELF_AWARENESS_TRACE_CONTEXT_LATEST_PATH = paths.artifacts["trace_context"]
    SELF_AWARENESS_FAILURE_MATRIX_LATEST_PATH = paths.artifacts["failure_matrix"]
    SELF_AWARENESS_WORKING_STACK_LATEST_PATH = paths.artifacts["working_stack"]
    SELF_AWARENESS_ACTIVATION_SMOKE_LATEST_PATH = paths.artifacts["activation_smoke"]
    SELF_AWARENESS_AUTOLINK_LATEST_PATH = paths.artifacts["autolink"]
    SELF_AWARENESS_EVENTS_LATEST_PATH = paths.artifacts["events"]
    SELF_AWARENESS_COLLECT_LATEST_PATH = paths.artifacts["collect"]
    SELF_AWARENESS_QUERY_LATEST_PATH = paths.artifacts["query"]
    SELF_AWARENESS_CORRELATION_LATEST_PATH = paths.artifacts["correlation"]
    SELF_AWARENESS_TIMELINE_LATEST_PATH = paths.artifacts["timeline"]
    SELF_AWARENESS_SPATIAL_GRAPH_LATEST_PATH = paths.artifacts["spatial_graph"]
    SELF_AWARENESS_CONTEXT_LATEST_PATH = paths.artifacts["context"]
    SELF_AWARENESS_EPISODES_LATEST_PATH = paths.artifacts["episodes"]
    SELF_AWARENESS_ALERTS_LATEST_PATH = paths.artifacts["alerts"]
    REACTIONS_LATEST_PATH = paths.artifacts["reactions"]
    RESPONSES_LATEST_PATH = paths.artifacts["responses"]
    SELF_AWARENESS_INVESTIGATE_LATEST_PATH = paths.artifacts["investigate"]
    SELF_AWARENESS_REPLAY_LATEST_PATH = paths.artifacts["replay"]
    SELF_AWARENESS_BRIEF_LATEST_PATH = paths.artifacts["brief"]
    SELF_AWARENESS_PROBE_LATEST_PATH = paths.artifacts["probe"]
    SELF_AWARENESS_COMPLETION_AUDIT_LATEST_PATH = paths.artifacts["completion_audit"]
    SELF_AWARENESS_VALIDATE_LATEST_PATH = paths.artifacts["validate"]
    SELF_AWARENESS_CYCLE_LATEST_PATH = paths.artifacts["cycle"]
    SELF_AWARENESS_COVERAGE_AUDIT_LATEST_PATH = paths.artifacts["coverage_audit"]
    SELF_AWARENESS_EXPORT_LATEST_PATH = paths.export_latest
    SELF_AWARENESS_EXPORT_ROOT = paths.export_history_root
    ...
    self_awareness_working_stack_inventory = refresh_port.working_stack
    self_awareness_stack_closure_dossier = refresh_port.stack_closure_dossier
    self_awareness_trace_context_fallback = refresh_port.trace_context_fallback
    self_awareness_objective_coverage_audit = refresh_port.coverage_audit
    self_awareness_activation_smoke = refresh_port.activation_smoke
    self_awareness_autolink = refresh_port.autolink
    self_awareness_completion_audit = refresh_port.completion_audit
    self_awareness_alerts = refresh_port.alerts
    reaction_status = refresh_port.reactions
    response_status = refresh_port.responses
    self_awareness_investigate = refresh_port.investigate
    self_awareness_replay = refresh_port.replay
    self_awareness_refresh_working_stack_dependent_readmodels = refresh_port.refresh_working_stack_dependents

    self_awareness_trace_context_fallback_complete = contract_port.trace_context_fallback_complete
    self_awareness_requirement_probes_cover_requirements = contract_port.requirement_probes_cover_requirements
    ...
    self_awareness_entity_event_document_map_complete = contract_port.entity_event_document_map_complete
    self_awareness_completion_route_packet_issues = contract_port.completion_route_packet_issues
    self_awareness_export_stack_handoff = contract_port.export_stack_handoff
    ...
    self_awareness_working_stack_link_integrity_matrix_complete = contract_port.working_stack_link_integrity_complete
    self_awareness_link_integrity_matches_working_stack = contract_port.link_integrity_matches_working_stack
    ...
    self_awareness_export_artifact_entry = lambda name, path, schema: export_artifact_entry(
        name,
        path,
        schema,
        runtime_port=runtime_port,
    )
    self_awareness_export_artifact_status_lists = export_artifact_status_lists
    self_awareness_refresh_exported_artifacts = lambda exported, specs, names: refresh_exported_artifacts(
        exported,
        specs,
        names,
        runtime_port=runtime_port,
    ...
    def self_awareness_include_coverage_audit_artifact(artifacts: dict[str, tuple[Path, str]]) -> None:
        artifacts["coverage_audit"] = (
            SELF_AWARENESS_COVERAGE_AUDIT_LATEST_PATH,
            f"{SCHEMA_PREFIX}_self_awareness_objective_coverage_audit_v1",
        )
    ...
    if runtime_port.path_exists(SELF_AWARENESS_COVERAGE_AUDIT_LATEST_PATH):
...
def stack_organ_use_packet_complete(
    ...
    *,
    ...
    return self_awareness_activation_contracts.stack_organ_use_packet_complete(
        packet,
        schema_prefix=schema_prefix,
...
def working_stack_activation_smoke_row_complete(
    ...
    *,
    ...
    return self_awareness_activation_contracts.working_stack_activation_smoke_row_complete(
        row,
        schema_prefix=schema_prefix,
...
def working_stack_activation_smoke_compact(
    ...
    *,
    ...
    return self_awareness_activation_contracts.working_stack_activation_smoke_compact(
        row,
        schema_prefix=schema_prefix,
        investigation_node_count=investigation_node_count,
        event_issues=event_issues,
...
def working_stack_activation_smoke_complete(
    ...
    *,
    ...
    return self_awareness_activation_contracts.working_stack_activation_smoke_complete(
        smoke,
        schema_prefix=schema_prefix,
        investigation_node_count=investigation_node_count,
        event_issues=event_issues,
...
def activation_smoke_needs_refresh(
    ...
    *,
    ...
    return self_awareness_activation_contracts.activation_smoke_needs_refresh(
        smoke,
        activation_entries,
        schema_prefix=schema_prefix,
        investigation_node_count=investigation_node_count,
...
def working_stack_organ_signal_route(service: str, organ: Mapping[str, Any]) -> dict[str, str]:
    return self_awareness_activation_contracts.stack_organ_signal_route(service, organ)


def working_stack_organ_state_digest(organ: Mapping[str, Any]) -> str:
    return self_awareness_activation_contracts.stack_organ_state_digest(organ)
...
def working_stack_organ_movement_selection(
    ...
    *,
    ...
    return self_awareness_activation_contracts.stack_organ_movement_selection(
        organ,
        current_state_digest=current_state_digest,
