...
- updateable/installable artifacts pass the OS Abyss TUF-style metadata gate,
  external TUF repository producer/verifier path, and artifact `trust-gate`
  verdict before update-client consumption; public-channel TUF key ceremony and
  live SCITT transparency service remain separate production layers;
