...
- hotpath probe document assembly and orchestration through fakeable residency,
  TTS/STT/LLM probe, AI-policy, path-existence, and monotonic ports.

The CLI still owns memory policy/path contracts, concrete hotpath TTS/STT/LLM
probe execution, candidate ranking, confirmation and authorization checks,
latest/history/index writes, concrete upstream port binding, and command
rendering. The adapter does not broaden mutation authority: without the
...
1. Storage/process/memory/mode/cooling adapters: continue with concrete
   ...
   Continue with concrete hotpath execution ports or broader container orchestration
   only when the fakeable live port is
   clear. Keep
