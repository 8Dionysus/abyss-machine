...
parser/result rendering, resident TTS server loop/model lifecycle, broader
resource sampling, and AI report assembly. The adapters do not download models,
mutate stack repositories, or publish generated host evidence. Resident service
mutation remains an explicit operator command routed through the resident
controller.
...
1. AI runtime adapters: resident TTS server loop/model lifecycle,
   ...
   execution, the TTS client/cold-synth runner, and TTS output audio/resource
