...
callbacks, fact routing, and command rendering. Browser-content BiDi/history
...
command rendering. Browser-content BiDi/history capture remains a separate
nervous-local capture debt.

## Extracted Nervous Browser-Content Store And AT-SPI Capture Seam

`abyss_machine.nervous_browser_content_adapters` owns the browser-content local
store, ingest seam, and Firefox AT-SPI accessibility-tree capture runtime:
...
source-policy/privacy decisions, BiDi/WebSocket and browser-history capture
execution, capture orchestration, and command rendering. Browser-content
BiDi/history capture execution remains a separate nervous-local adapter debt.
