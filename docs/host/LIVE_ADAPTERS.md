...
The CLI still owns memory policy/path contracts, hotpath TTS/STT/LLM probe
execution, candidate ranking, confirmation and authorization checks,
...
1. Storage/process/memory/mode/cooling adapters: continue with concrete
   ...
   first mode mutation adapter and cooling host-control adapter. Continue with
   remaining mode reconcile orchestration, cooling live sampling/validation
   orchestration, or hotpath execution only when the fakeable live port is
   clear. Keep
