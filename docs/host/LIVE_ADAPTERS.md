...
orchestration, token-accounting profile selection/latest writes, resident TTS
server loop/model lifecycle, TTS audio file summary/resource-report assembly,
resident LLM execution, and AI report assembly. The adapters do not download models, launch
...
1. AI runtime adapters: resident TTS server loop/audio summary execution,
   resident LLM execution, benchmark/eval suite orchestration, and broader
   resource sampling. Runtime/model discovery, bounded OpenVINO benchmark/eval
   runners, token-accounting tokenizer execution, and the TTS client/cold-synth
   runner are already split into `ai_runtime_adapters`/`ai_tts_adapters`;
