...
The CLI still owns memory policy/path contracts, candidate ranking,
confirmation and authorization checks, latest/history/index writes, concrete
upstream port binding, and command rendering. The adapter does not broaden
...
1. Storage/process/memory/mode/cooling adapters: continue with concrete
   ...
   Continue with broader container orchestration or remaining candidate,
   authorization, and write-routing seams only when the fakeable live port is
   clear. Keep
