...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   partial/building/final result document assembly plus read-only systemd
   timer/service discovery, state normalization, and observation-event
   orchestration now live in
   `self_awareness_adapters`; split remaining
   probe/cycle/replay/investigate execution, concrete stack path binding/latest
   reads, and deeper runtime refresh orchestration only when the next fakeable
   live port proves a reusable center still sits in CLI.
