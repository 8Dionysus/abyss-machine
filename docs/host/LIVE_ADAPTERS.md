...
- TTS Unix-socket JSON-line client transport, server status/stop request
  exchanges, synth subprocess cache/env binding, and BabelVox/Qwen3 OpenVINO
  cold synth child-process invocation through fakeable socket, runner, path, and
  environment ports.
...
parser/result rendering, resident TTS server loop/model lifecycle, TTS audio
file summary/resource-report assembly, and AI report assembly. The adapters do
...
1. AI runtime adapters: resident TTS server loop/audio summary execution,
   ...
   execution, and the TTS client/cold-synth runner are already split into
   `ai_runtime_adapters`/`ai_tts_adapters`; dictation should only reappear here
   when a concrete remaining CLI edge has been re-inventoried.
