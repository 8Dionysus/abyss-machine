...
dictation status/validation readiness execution. Those remain explicit live adapter
...
flow, validation/latest writes, broader status command readiness, and command
rendering. The adapters do not persist WAV files in Git or make model weights
...
2. Dictation and AI runtime adapters: dictation status/validation writes and AI
