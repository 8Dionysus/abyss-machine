...
supplied contract ports. The CLI retains final latest/history/index writes.
The adapter does not mutate the stack, start services, authorize remediation,
or perform persistence. Recovery is to retry the failed owner collector or
reuse its latest document; no destructive rollback exists.
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   acquisition, and collect event/fabric/readmodel assembly now live in
   `self_awareness_adapters`; split remaining
   final collect writes, probe/cycle/replay/investigate
   execution, concrete artifact/freshness binding, and deeper runtime
