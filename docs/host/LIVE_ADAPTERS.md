...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   projections, and cycle artifact evidence snapshots now live in
   `self_awareness_adapters`; split remaining probe/cycle/replay/investigate
   execution, concrete stack path binding/latest reads, and deeper runtime
   refresh orchestration only when the next fakeable live port proves a reusable
