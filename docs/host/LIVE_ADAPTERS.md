...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   `self_awareness_adapters`; complete probe and cycle preflight/refresh/proof/
   two-stage-persistence orchestration also lives there. Split remaining
   concrete artifact/freshness binding and deeper runtime
   orchestration only when the next fakeable live port proves a reusable center
   still sits in CLI.
