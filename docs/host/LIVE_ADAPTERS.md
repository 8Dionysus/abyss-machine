...
| `self-awareness` | stack/runtime latest files, observability probes, generated event/fabric stores, systemd state. | self-awareness timeline/context/episode/brief/query/probe/latest surfaces. | probe/cycle/replay/investigate orchestration and stack handoff checks. | `self_awareness_contracts` owns read-model shapes; `self_awareness_adapters` owns latest surface specs, fakeable latest load dispatch, validation latest-spec selection, and bounded status latest summaries; live execution/refresh orchestration remains CLI edge. |
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch
   and bounded status summaries now live in `self_awareness_adapters`; split
   probe/cycle/replay/investigate execution and deeper refresh orchestration
   only when a fakeable live probe port proves a reusable center still sits in
   CLI.
