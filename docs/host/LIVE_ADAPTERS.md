...
| `self-awareness` | stack/runtime latest files, observability probes, generated event/fabric stores, systemd state. | self-awareness timeline/context/episode/brief/query/probe/latest surfaces. | capabilities/status/export/probe/investigate/replay/cycle caller binding and stack handoff checks. | Package modules own general read-model, completion, export-handoff, validation, causal timeline/spatial/context/episode, stack requirement/closure-dossier, and bounded live stack-probe pipelines. `self_awareness_adapters`, `self_awareness_causal_readmodel_contracts`, `self_awareness_stack_closure_contracts`, and `self_awareness_stack_probe_adapters` own their extracted orchestration or runtime boundaries through explicit fakeable ports. CLI binds concrete paths, live functions, URLs, runners, contract callbacks, and operator write intent. Remaining direct-IO helpers still require classification before family closure. |
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   closed, extract the remaining causal overlays/readmodels, failure matrix,
   and activation/lineage/coverage centers, then classify every remaining
   direct-IO CLI helper as a concrete
