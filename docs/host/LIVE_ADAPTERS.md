...
orchestration, resident TTS server loop/model lifecycle, TTS audio file
summary/resource-report assembly, resident LLM execution, tokenizer subprocess
execution, and AI report assembly. The adapters do not download models, launch
resident services, mutate stack repositories, or publish generated host
evidence.
...
1. AI runtime adapters: remaining runtime subprocess plans, resident TTS server
   loop/audio summary execution, resident LLM execution, and tokenizer
   subprocess execution. Runtime/model discovery, bounded OpenVINO
   benchmark/eval runners, and the TTS client/cold-synth runner are already
   split into `ai_runtime_adapters`/`ai_tts_adapters`; dictation should only
   reappear here when a concrete remaining CLI edge has been re-inventoried.
