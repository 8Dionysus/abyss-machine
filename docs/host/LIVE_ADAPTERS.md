...
## Extracted Self-Awareness Collect Input Seam
...
The CLI binds concrete paths and current subsystem/network/time functions, then
owns event/fabric assembly and final latest/history/index writes. The adapter
does not inspect raw private payloads beyond the documents returned by those
ports, mutate the stack, start services, or authorize remediation. Recovery is
to retry the failed owner collector or reuse its latest document; no destructive
rollback exists.
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   timer/service discovery/state/event orchestration, and ordered collect input
   acquisition now live in
   `self_awareness_adapters`; split remaining
   collect event/fabric assembly and final writes, probe/cycle/replay/investigate
