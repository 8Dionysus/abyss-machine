...
| `self-awareness` | stack/runtime latest files, observability probes, generated event/fabric stores, systemd state. | self-awareness timeline/context/episode/brief/query/probe/latest surfaces. | capabilities/status/export/probe/investigate/replay/cycle caller binding and stack handoff checks. | Package modules own general read-model, completion, export-handoff, validation, causal timeline/spatial/context/episode, and stack requirement/closure-dossier pipelines. `self_awareness_adapters` owns the extracted live orchestration families; `self_awareness_causal_readmodel_contracts` and `self_awareness_stack_closure_contracts` own their readmodel refresh/assembly/latest-history routes through explicit fakeable ports. CLI binds concrete paths, live functions, URLs, runners, contract callbacks, and operator write intent. Remaining direct-IO helpers still require classification before family closure. |
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   closed, extract the remaining causal overlays/readmodels, live stack
   capability/evidence probes, failure matrix, and activation/lineage/coverage
   centers, then classify every remaining direct-IO CLI helper as a concrete
