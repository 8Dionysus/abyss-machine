...
| `dictation` | audio devices, runtime config, transcripts, WAV metadata, server state. | transcript latest/JSONL, dictation index, validation latest. | recording, server transport, clipboard/text insertion, audio runtime subprocesses. | `dictation_contracts` owns shapes; `dictation_execution_adapters` owns explicit-file transcription via warm-server/helper runtime, including client-side 16 kHz preprocessing. Recording lifecycle, audio-doctor probes, clipboard insertion, and journal writes remain CLI edge. |
...
dictation recording/insertion/journal execution. Those remain explicit live
...
flow, recording start/stop lifecycle, clipboard/text insertion, transcript
journal writes, latest/index writes, and command rendering. The adapter does
not persist WAV files in Git or make model weights public; runtime audio copies
