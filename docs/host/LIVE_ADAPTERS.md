...
callbacks, fact routing, and command rendering. Browser-content live capture
remains a separate nervous-local adapter debt.
...
command rendering. Browser-content live capture remains the next nervous-local
capture debt.

## Extracted Nervous Browser-Content Store Seam

`abyss_machine.nervous_browser_content_adapters` owns the browser-content local
store and ingest seam:
...
The CLI still owns concrete path constants, clock/user binding, AT-SPI/BiDi
capture runtime callbacks, source-policy/privacy decisions, capture orchestration,
and command rendering. Browser-content AT-SPI/BiDi/history capture execution
remains a separate nervous-local adapter debt.
