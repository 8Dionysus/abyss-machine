...
The CLI still owns concrete `/etc` config loading, command rendering, concrete
policy readmodel collection, token-accounting live text input binding, and
concrete live reader/env source selection for the core AI readmodels.
...
1. AI runtime adapters: concrete policy readmodel collection and any remaining
   resident LLM readmodel/write orchestration that is still proven to sit in `cli.py`.
   ...
   store/readmodel routing, resource snapshot/profile assembly, subprocess env
   binding, resident and workhorse LLM controller execution, STT eval
   ...
   `ai_runtime_adapters`/`ai_tts_adapters`; concrete live readers stay in CLI
   unless a reusable adapter center is proven;
