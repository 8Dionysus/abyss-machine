...
selection for the core AI readmodels.
...
1. AI runtime adapters: any remaining resident LLM readmodel/write orchestration
   that is still proven to sit in `cli.py`.
   ...
   store/readmodel routing, policy input collection, resource snapshot/profile
   assembly, subprocess env binding, resident and workhorse LLM controller execution, STT eval
