...
| `self-awareness` | stack/runtime latest files, observability probes, generated event/fabric stores, systemd state. | self-awareness timeline/context/episode/brief/query/probe/latest surfaces. | capabilities/status/export/probe/investigate/replay/cycle caller binding and stack handoff checks. | Package modules own general read-model, completion, export-handoff, validation, and causal timeline/spatial/context/episode pipelines. `self_awareness_adapters` owns the extracted live orchestration families; `self_awareness_causal_readmodel_contracts` owns causal readmodel assembly, refresh fallback, and latest/history routing through explicit fakeable ports. CLI binds concrete paths, live functions, URLs, runners, contract callbacks, and operator write intent. Remaining direct-IO helpers still require classification before family closure. |
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   now live in `self_awareness_causal_readmodel_contracts`. Before declaring
   this family closed, extract the remaining causal overlays/readmodels and
   stack capability/requirement/closure plus activation/lineage/coverage
