...
This is a contract boundary, not a completed live adapter. The CLI still owns
status/latest/resource/artifact input collection, latest/history persistence,
and rendering. Public tests constrain
...
The CLI supplies already-loaded values and retains concrete latest/history
persistence and write-failure projection. Public tests constrain backlog
...
2. Self-awareness live orchestration adapters: latest-read specs/load dispatch,
   ...
   `self_awareness_completion_document_contracts`. Split remaining completion-
   audit live input/persistence orchestration, validation orchestration, plus
   the stable export stack-handoff contract builder, only when the next
   evidence-led slice proves a reusable center still sits in CLI.
