...
- Semantic-build window execution seam:
  ...
  meta values, DB mode, and counts. Follow-up semantic-search/eval seams move
  search limit/privacy/preflight/query-vector projection, vector-search
  dispatch, eval probe/check/result assembly, eval-query embedding fanout,
  lexical/semantic eval search dispatch, and semantic-eval latest/history
  routing through fakeable ports. CLI still owns concrete config/path/model/
...
- Quality-audit live input seam: `abyss_machine.nervous_quality_adapters` owns
  quality refresh orchestration, validation/status/capture/privacy/source/
